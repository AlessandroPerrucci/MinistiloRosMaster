...
class Rosmaster(object):
    ...
    #         print('---set_pid_param error!---')
    #         pass
    def set_car_motion_custom(self, v_x, v_y, v_z, right_gain=1.3):
        """
        v_x, v_y, v_z: stessi significati di set_car_motion
        right_gain: moltiplicatore applicato ai motori di destra
        """
        # 1) calcolo “raw” delle velocità di ogni ruota
        #    (queste formule sono quelle tipiche per un carrello omnidirezionale 4-ruote)
        m1 = v_x - v_y - v_z  # ruota avanti sinistra
        m2 = v_x + v_y - v_z  # ruota dietro  sinistra
        m3 = v_x + v_y + v_z  # ruota avanti destra
        m4 = v_x - v_y + v_z  # ruota dietro  destra

        # 2) applica il guadagno extra sui motori di destra
        m3 *= right_gain
        m4 *= right_gain

        # 3) normalizza (opzionale, se superi ±1.0)
        ...
        # 4) invoca set_motor convertendo in [-100, +100]
        self.set_motor(
            int(m1 * 100),
            int(m2 * 100),
            int(m3 * 100),
...
if __name__ == '__main__':
    ...
    print(motor_encoders[0] - motor_encoders_prima[0], "------", motor_encoders[2] - motor_encoders_prima[2], "\n",
          motor_encoders[1] - motor_encoders_prima[1], "------", motor_encoders[3] - motor_encoders_prima[3])
